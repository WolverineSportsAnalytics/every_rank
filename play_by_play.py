...
# Helper function to get starters to the lineup can start with something
# TODO handle broken lineups
...
def _order_players(on_court  : List[str],
    ...
    while i <= high:
        ...
        # so we default to guard in this case. See 'oel Ofori of new Middlebury
        if not position:
            position = "G"
...
def _fix_glitch(table : pd.DataFrame, game : pd.DataFrame) -> pd.DataFrame:
    if int(table[list(table.columns)][3][1]) != game["Away_Score"].iloc[-1]:
...
def scrape_game(game_id : int) -> pd.DataFrame:
    ...
    # 20 is arbitary but will save us up to 18 OTs or 16 in womens so it should
    # be fine
    if len(game) < 20:
